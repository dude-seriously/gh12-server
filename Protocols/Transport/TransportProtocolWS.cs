...
namespace Server.Protocols.Transport {
	/*[ATransportProtocol("websocket", 0)]
    sealed class TransportProtocolWSDraft:ITransportProtocol {
        static private ITransportProtocol instance = new TransportProtocolWSDraft();

        private TransportProtocolWSDraft() { }
	...
	/*} catch(Exception ex) {
        ...
        public byte[] Send(byte[] binary) {
            try {
                ...
                var wrappedBytes = new byte[binary.Length + 2];
                wrappedBytes[0] = 0;
                wrappedBytes[wrappedBytes.Length - 1] = 255;
                Array.Copy(binary, 0, wrappedBytes, 1, binary.Length);
                return wrappedBytes;
